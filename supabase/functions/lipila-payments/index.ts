...
// ============================================
// Lipila Payment Gateway — Edge Function (Fastele)
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.action === "status") {
      ...
      const ep = getEndpoint("status", config.baseUrl, paymentMethod, body.referenceId);
    ...
    const referenceId = body.referenceId || `fastele-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    if (body.action === "collect" && paymentMethod === "card") {
      ...
      const email = body.email || `${phone}@fastele.app`;
      lipilaBody = {
        customerInfo: {
          firstName,
          lastName,
        ...
        collectionRequest: {
          referenceId,
          amount: body.amount,
          narration: body.narration || "Fastele escrow",
          accountNumber: phone,
          currency: body.currency || "ZMW",
          backUrl: body.backUrl || config.callbackUrl || "https://fastele.app",
          redirectUrl: body.redirectUrl || config.callbackUrl || "https://fastele.app",
        },
      };
      ...
      lipilaBody = {
        ...
        amount: body.amount,
        currency: body.currency || "ZMW",
        narration: body.narration || "Fastele bank payout",
        accountNumber,
        swiftCode,
        firstName,
        lastName,
      ...
      if (body.action === "collect") {
        lipilaBody.narration = body.narration || "Fastele escrow";
